 ...
 *
 * A single user can have many active subscriptions (desktop Chrome, Android
 * Chrome, Firefox, …). Each maps to one browser `PushSubscription`. The
 * `endpoint` is the push-service capability URL and must be treated as
 * sensitive data (it is the only thing needed to deliver a push).
 */
export const pushSubscriptions = sqliteTable(
	"push_subscriptions",
	{
		...
		/** Push-service endpoint (capability URL). Unique across users. */
