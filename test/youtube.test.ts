...
test("expandYouTubeMarkers replaces markers with iframes", () => {
	const out = expandYouTubeMarkers('<p><youtube-embed data-id="MmVoWzSyF9c"></youtube-embed></p>');
	expect(out).toBe(`<p>${youtubeEmbedHtml("MmVoWzSyF9c")}</p>`);
	// Invalid ids are left untouched.
	expect(expandYouTubeMarkers('<youtube-embed data-id="bad"></youtube-embed>')).toBe(
		'<youtube-embed data-id="bad"></youtube-embed>',
	);
});
...
test("pipeline with a strict sanitizer only allows our generated iframes", () => {
	...
	// handlers and javascript: urls but keeps custom elements.
